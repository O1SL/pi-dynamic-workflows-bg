...
| Artifacts | Implemented | Each run writes `status.json`, `events.jsonl`, `output.md`, `result.json`, and child sessions under `sessions/`. Key JSON/markdown artifacts are written via temp-file + atomic rename, with best-effort temporary-file cleanup on failed writes. |
