...
| Artifacts | Implemented | Each run writes `status.json`, `events.jsonl`, `output.md`, `result.json`, and child sessions under `sessions/`. `snapshot.graph` provides a best-effort display graph for pi-web with stable agent node ids, phase/status, attempts, tool/session/worktree metadata, simple seq edges, and nested parallel/pipeline groups. Key JSON/markdown artifacts are written via temp-file + atomic rename, with best-effort temporary-file cleanup on failed writes. |
