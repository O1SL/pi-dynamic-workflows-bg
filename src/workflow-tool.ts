...
export function createWorkflowTool(options: WorkflowToolOptions = {}): ToolDefinition<typeof workflowToolSchema, any> {
  return defineTool({
    ...
    label: "Workflow",
    description: [
      "Run deterministic JavaScript workflows that orchestrate multiple subagents.",
      "Use workflow for explicit workflow/fan-out/multi-agent orchestration, dynamic if/loop decisions, parallel batches, pipelines, and result synthesis.",
      "Core lifecycle: background by default, foreground:true only for inline waits; use workflow_wait to consume results, workflow_status/result to inspect progress and outcomes, and workflow_cancel to stop a running run.",
      "Follow-up lifecycle: workflow_extend starts a linked workflow from parent context; workflow_replace_tail cancels and replaces a running parent; workflow_resume continues one child session only.",
      "Diagnostics and operations: workflow_summary/events/transcript inspect state and child sessions; workflow_worktrees/worktree_cleanup manage isolated worktrees; workflow_prune cleans old terminal artifacts; workflow_steer sends experimental live guidance to a running child.",
      "Workflow scripts use agent(), parallel(), pipeline(), phase(), log(), args, cwd, process.cwd(), and budget. Child agents support labels, schema output, model/fallback/retry, timeout, tool/turn budgets, and worktree isolation.",
      "Before advanced authoring, interruption recovery, or choosing among workflow/subagents/intercom, read the `pi-dynamic-workflows-bg` skill for detailed routing and usage guidance.",
    ].join(" "),
    promptSnippet:
    ...
    promptGuidelines: [
      "Use workflow only when the user explicitly asks for a workflow, workflows, fan-out, or multi-agent orchestration. Read the `pi-dynamic-workflows-bg` skill before advanced authoring, interruption recovery, or choosing among workflow/subagents/intercom.",
      ...
      "Use agent(prompt, opts) for child work. Important opts: label, phase, schema, model, fallbackModels, retry/retryDelayMs, timeoutMs, toolBudget, turnBudget, isolation:'worktree', agentType (role hint only).",
      "Available globals are agent(prompt, opts), parallel(thunks), pipeline(items, ...stages), phase(title), log(message), args, continuation, cwd, process.cwd(), and budget. continuation is read-only and exists only in workflows started via workflow_extend or workflow_replace_tail; use continuation.parent.runId/result/outputPath/snapshot, not continuation.id/result. Every workflow must call agent() at least once; do not use workflow only to declare phases or return a static object.",
      ...
      "When early results require new follow-up work, prefer workflow_extend to start a linked workflow with read-only continuation context. Use workflow_replace_tail only when a running parent must be cancelled and replaced; it validates the replacement script before cancelling. Never try to mutate a running workflow script. Do not confuse workflow_resume, which continues one child session, with extending or replacing a workflow run. Read the `pi-dynamic-workflows-bg` skill before using these lifecycle tools.",
      "Use workflow_status/result/summary to inspect run state, workflow_events to inspect lifecycle, workflow_transcript to inspect a child session, workflow_wait when this turn must consume the result, and workflow_cancel only to stop running work. Read the `pi-dynamic-workflows-bg` skill before deeper diagnostics or follow-up recovery.",
    ],
